...
<div class="space-y-8 animate-fade-in max-w-5xl mx-auto p-4 sm:p-0">
    <div class="px-2">
        ...
        <h1 class="text-2xl sm:text-3xl font-poppins font-bold text-[var(--text-main)]">Edit Detail Pengguna</h1>
        <p class="text-[var(--text-muted)] text-xs sm:text-sm font-medium mt-1">Perbarui profil pengguna — perubahan langsung sinkron ke Web & Android.</p>
    </div>
    ...
    <div class="bg-[var(--bg-card)] p-5 sm:p-10 rounded-3xl sm:rounded-[2.5rem] shadow-[var(--card-shadow)] border border-[var(--border)] relative overflow-hidden">
        <form action="{{ route('admin.users.update', $user) }}" method="POST" enctype="multipart/form-data" class="relative z-10 space-y-8">
            @csrf
            @method('PUT')
                ...
                <div class="space-y-3">
                    ...
                    <div class="flex items-center space-x-4 p-2 bg-[var(--bg-main)] border border-[var(--border)] rounded-2xl">
                        <label class="flex-1 flex items-center justify-center space-x-2 px-4 py-3 rounded-xl cursor-pointer transition-all has-[:checked]:bg-[var(--bg-card)] has-[:checked]:shadow-sm has-[:checked]:text-emerald-500 text-[var(--text-muted)]">
                            <input type="radio" name="role" value="user" class="hidden" {{ old('role', $user->role) == 'user' ? 'checked' : '' }}>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div class="space-y-3">
                    <label class="text-[10px] font-black text-[var(--text-muted)] uppercase tracking-[0.2em] ml-1">Kata Sandi Baru <span class="text-[9px] text-[var(--text-muted)]/50 normal-case font-bold">(Kosongkan jika tak ingin diubah)</span></label>
                ...
                <div class="space-y-3">
                    <label class="text-[10px] font-black text-[var(--text-muted)] uppercase tracking-[0.2em] ml-1">Konfirmasi Kata Sandi Baru</label>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div class="space-y-3">
                    ...
                    <textarea name="address" rows="5"
                        ...
                        placeholder="Tulis alamat tempat tinggal lengkap...">{{ old('address', $user->address) }}</textarea>
                </div>
                        ...
                        <div onclick="document.getElementById('photo-input').click()"
                            ...
                            <div id="photo-preview" class="flex flex-col items-center justify-center p-4">
                                @if($user->profile_photo && file_exists(public_path('uploads/profile/' . $user->profile_photo)))
                                    <img src="{{ asset('uploads/profile/' . $user->profile_photo) }}" class="w-28 h-28 object-cover rounded-2xl shadow-lg border border-[var(--border)]">
                                @else
                                    <svg class="w-10 h-10 text-[var(--text-muted)] mb-2 group-hover:text-emerald-500 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
                                    <span class="text-xs font-bold text-[var(--text-muted)] group-hover:text-emerald-600 transition-colors">Pilih Foto Baru</span>
                                @endif
                            </div>
                        </div>
                    </div>
